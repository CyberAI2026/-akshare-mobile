...
def _series_metrics(g: pd.DataFrame) -> dict:
    ...
    v = pd.to_numeric(g["成交量"], errors="coerce") if "成交量" in g else pd.Series(np.nan, index=g.index)
    turnover = pd.to_numeric(g["换手率"], errors="coerce") if "换手率" in g else pd.Series(np.nan, index=g.index)
    if c.dropna().empty:
        return {}
    last = float(c.iloc[-1])
    def ret(n):
    ...
    vol5 = float(v.tail(5).mean()) if v.tail(5).notna().any() else np.nan
    vol_prev10 = float(v.iloc[-15:-5].mean()) if len(v) >= 15 and v.iloc[-15:-5].notna().any() else np.nan
    vol_contraction = vol5 / vol_prev10 if pd.notna(vol5) and pd.notna(vol_prev10) and vol_prev10 > 0 else np.nan
    turnover5 = float(turnover.tail(5).mean()) if turnover.tail(5).notna().any() else np.nan
    turnover_prev10 = float(turnover.iloc[-15:-5].mean()) if len(turnover) >= 15 and turnover.iloc[-15:-5].notna().any() else np.nan
    turnover_contraction = turnover5 / turnover_prev10 if pd.notna(turnover5) and pd.notna(turnover_prev10) and turnover_prev10 > 0 else np.nan
    ...
    if convergence_available < 2:
        consolidation_state = "证据不足"
    elif convergence_support >= 3 and short_decline_stopped:
        consolidation_state = "收敛较充分"
    elif convergence_support >= 2:
        consolidation_state = "部分收敛/待确认"
    else:
        consolidation_state = "结构未成熟"
...
def stage2_rank(metrics: pd.DataFrame, min_n: int = 30, max_n: int = 40, return_audit: bool = False):
    ...
    for c in ["ret1", "ret10", "ret40", "amp5", "MA20距离", "MA30_5日斜率", "距稳健5日上沿",
              "振幅收敛比", "成交量收敛比", "换手率收敛比", "MA5_3日斜率", "MA10_3日斜率", "整理证据可用项", "整理收敛支持项"]:
        if c not in x.columns:
            x[c] = np.nan
        x[c] = pd.to_numeric(x[c], errors="coerce")

    x["基础整理条件"] = (x["amp5"] <= 0.18) & (x["ret10"] <= 0.20)
